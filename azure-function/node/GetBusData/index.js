...
module.exports = async function (context, myTimer) {
  ...
  await Promise.all(activatedGeofences.map(async (gf) => {
    context.log(`Vehicle ${gf.VehicleId}, route ${gf.RouteId}, ${gf.GeoFenceStatus} Geofence ${gf.GeoFence} at ${gf.TimestampUTC} UTC.`);
...
async function TriggerLogicApp(context, geofence) {
  ...
  try {
    ...
    if (response.ok) context.log(`[${geofence.VehicleId}/${geofence.DirectionId}/${geofence.GeoFenceId}] WebHook called successfully`);
    else context.log(`Error calling Logic App. HTTP Response is: ${response.status}`);
  } catch (err) {
